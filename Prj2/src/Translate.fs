...
let rec transExp (symtab: SymbolTable) (e: Exp) : Register * Instr list =
  ...
  | And (e1, e2) ->
    ...
    let t4 = createRegName ()
    let (t1, i1) = transExp symtab e1
    let (t2, i2) = transExp symtab e2
    let L1 = createLabel ()
    let L2 = createLabel ()
    let if_i = i1 @ [GotoIfNot (Reg t1, L1)] @ i2
    (t4, if_i @ [Set (t3, Reg t2); Goto (L2); Label (L1); Set (t3, Imm 0); Label (L2); Set (t4, Reg t3)])
  ...
  | Or (e1, e2) ->
    ...
    let L1 = createLabel ()
    let L2 = createLabel ()
    let if_i = i1 @ [GotoIf (Reg t1, L1)] @ i2
    (t4, if_i @ [Set (t3, Reg t2); Goto (L2); Label (L1); Set (t3, Imm 1); Label (L2); Set (t4, Reg t3)])
